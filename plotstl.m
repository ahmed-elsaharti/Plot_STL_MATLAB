function [xbounds,ybounds,zbounds] = plotstl(filename,x,y,z,scale,surfcolor,alpha)
...
% Transfer points to a read-write obj and move file to origin
    CustomObj(:,1)=Originalobj.Points(:,1)-min(Originalobj.Points(:,1));
    CustomObj(:,2)=Originalobj.Points(:,2)-min(Originalobj.Points(:,2));
    CustomObj(:,3)=Originalobj.Points(:,3)-min(Originalobj.Points(:,3));
